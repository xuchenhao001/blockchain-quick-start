...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "putExplicitCollection":
		return t.putExplicitCollection(stub, args)
	case "readExplicitCollection":
		return t.readExplicitCollection(stub, args)
	case "readCollectionHash":
	    return t.readCollectionHash(stub, args)
	default:
		//error
...
func (t *SimpleChaincode) putExplicitCollection(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	...
	// === Save marble to state ===
	err = stub.PutPrivateData("_implicit_org_Org1MSP", marbleInput.Name, marbleJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *SimpleChaincode) readExplicitCollection(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var name, jsonResp string
	var err error
	...
	name = args[0]
	valAsbytes, err := stub.GetPrivateData("_implicit_org_Org1MSP", name) //get the marble from chaincode state
	if err != nil {
		jsonResp = "{\"Error\":\"Failed to get state for " + name + ": " + err.Error() + "\"}"
		return shim.Error(jsonResp)
	} else if valAsbytes == nil {
...
func (t *SimpleChaincode) readCollectionHash(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var name, jsonResp, collection string
	var err error
	...
	name = args[1]
	collection = args[0]
	valAsbytes, err := stub.GetPrivateDataHash(collection, name) //get the marble from chaincode state
